...
class MultiChating(QWidget, form_class):
    ...
    def receive_message(self, so):
        while True:
            ...
            # if 'ch##' == json.loads(buf.decode('utf-8'))[0:4]:
            #     print('채팅방 목록')
            #     self.tableWidget.setRowCount(len(json.loads(buf.decode('utf-8'))))
            #     self.tableWidget.setColumnCount(1)
            #     for i in range(len(json.loads(buf.decode('utf-8')))):
            #         for j in range(1):
            #             self.tableWidget.setItem(i, j, QTableWidgetItem(str(json.loads(buf.decode('utf-8')))[i]))
        so.close()
