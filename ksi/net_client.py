...
class ChatClient(QWidget, form_class):
    ...
    def go_main(self):
        ...
        if self.sender_name =='':
            ...
            self.cursor.execute(f"insert into online_user (이름) values ('{self.sender_name}')")
            self.conn.commit()
            ip = '10.10.21.104'
            port = 9050
            ...
            listen_thr.start()
            self.le_name.clear()
    def online_user_update(self):
        self.lw_online_user.clear()
        conn = pymysql.connect(host='localhost', port=3306, user='root', password='00000000', db='net', charset='utf8')
        cursor = conn.cursor()
        cursor.execute(f"select * from online_user")
        online_user_list = cursor.fetchall()
    ...
    def closeEvent(self, QCloseEvent):
        conn = pymysql
        cursor = conn.cursor()
        cursor.execute(f"delete from online_user where 이름='{self.sender_name}'")
        conn.commit()
        conn.close()
        exit_msg = (f"{self.sender_name}/!&%*|CLOSE|*%&!").encode('utf-8')
        self.Thread_exit = True
        self.client_socket.send(exit_msg)
        QCloseEvent.accept()
...
class listen_Qthread(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            print(buf)
            if '@!|USER CONNECT|!@' in buf or '!&%*|CLOSE|*%&!' in buf:
                print('시발 왜안되나')
                self.parent.online_user_update()
                continue
            elif '!@|CHATING ROOM OPEN|@!' in buf:
                print(buf)
                self.parent.chat_room_update()
                continue
            self.parent.lw_message.addItem(buf)
