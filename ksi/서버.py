        ...
        while True:
            try:
                ...
                try:
                    ...
                    if user_name not in self.clients_name or f"!&%*|EXIT|*%&!" in msg or f'!&%*|CLOSE|*%&!' in msg:  # 새로 연결된 소켓이거나 서버에서 보낸 메시지가 exit or close 일 경우
                    ...
                    elif "!@#|DELETE CHAT ROOM|#@!" in msg:  # 유저가 채팅방을 삭제 했을 경우
                        chat_room_name = msg.split('\/')[2]  # 해당 채팅방이름 담아두기
                        cursor.execute(f"delete from chat_list where 채팅방이름='chat_room_{chat_room_name}'")
                        cursor.execute(f"drop table chat_room_{chat_room_name}")
                        conn.commit()
                        ...
                        self.final_received_message = f"{user_name}\/!&%*|UPDATE CHAT ROOM|*%&!\/{chat_list}\/{chat_room_name}".encode()
