            ...
            try:
                ...
                try:
                    ...
                    if user_name not in self.clients_name or f"!&%*|EXIT|*%&!" in msg or f'!&%*|CLOSE|*%&!' in msg: #새로 연결된 소켓이거나 서버에서 보낸 메시지가 exit or close 일 경우
                        ...
                        cursor.execute(f"insert into chat_room_{chat_room_name} values('{date}','{time}','{user_name}','님이 채팅방을 나가셨습니다.','{chat_room_name}','퇴장')")
                        ...
                        cursor.execute(f"insert into chat_room_{chat_room_name} values('{date}','{time}','{user_name}','님이 입장하셨습니다.','{chat_room_name}','입장')")
                        cursor.execute(f"update chat_list set 참여인원= 참여인원+1 where 채팅방이름='chat_room_{chat_room_name}'")
                        conn.commit() #수정값 저장
